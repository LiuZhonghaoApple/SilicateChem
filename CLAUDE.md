...
### 2026-08-01 域名整合：主推 silicatechem.com（运营已决策）

详见 [`seo/backlinks/2026-08-01-domain-conflict-zzhchem.md`](seo/backlinks/2026-08-01-domain-conflict-zzhchem.md)。

**背景（运营确认）**：龙港化工科技经营出问题，已更换经营主体；接手团队将出口业务授权本团队，其余网站陆续交接。**SEO 主推站点确定为 `silicatechem.com`。**

**现状**：众智名下三个站——`zzhchem.cn`（中文官网，工商登记官网 `lgwjg.com` 跳转至此）、`zzhchem.com`（英文站，与本站同产品同关键词，11 个产品页高度重叠、挂同名证书 PDF）、`silicatechem.com`（本仓库）。

**Bing 外链实测（2026-08-01）**：`silicatechem.com` 0 引用域；`zzhchem.com` **1**（唯一来源是 `zzhchem.cn`，7 条链接）；`zzhchem.cn` 0；对照 `pqcorp.com` 397。

> **据此纠正一个先前猜测**：曾设想"zzhchem.com 若已攒下外链，301 过来等于白捡权重"。实测不成立——它自己也几乎没有外部外链。**整合的价值在于停止关键词自相残杀、把后续力气归一，而非继承权重。** 不要期待整合带来排名跳升。

**交接后执行（四步，按序）**：① `zzhchem.com` 整站**逐页** 301 到 `silicatechem.com`（无对应页的钾/锂硅酸盐、正硅酸钠、阻燃剂建议先把内容搬过来再跳；**不要直接删站**）；② `zzhchem.cn` 上现有 7 条指向 `zzhchem.com` 的链接改指本站，并加 "English" 入口；③ 把 `zzhchem.com` 加入 GSC（目前账号内只有本站一个资源）；④ 证书 PDF 去重。

**交接前**：外链工作照常，三站外链均为 0，无可稀释者；但两站仍抢同一批词，排名会被压着，属暂时状态。

**⚠️ 待运营确认的阻塞项**：经营主体已变更，则**对外注册目录/投稿署名应使用哪个法律主体**？资料包现按工商登记的「山东众智化工科技有限公司」（信用代码 913707865677295728）填写。**确认前不要去注册目录类平台**——一旦被各平台抓取复制，更正极难。记者问答平台不受影响（用个人专家身份，不填执照信息）。

**另记**：工商信息带「被执行人」公开标签，与经营主体变更背景相符。相关性仅在于个别协会/目录收录前的背景核查。
...
- **真正的快车道是记者问答平台**：HARO/Connectively 已于 2024 年底关停。当前免费可用（2026-08-01 逐个打开核实）：**Source of Sources**（`sourceofsources.com`，无需注册账号，首页填姓名+邮箱订阅即可）、**MentionMatch**（`mentionmatch.com/register`；原 `helpab2bwriter.com` 已跳转更名，**"Help a B2B Writer" 这个名字已过期**）、Qwoted（`app.qwoted.com/users/sign_up`）。拿到的是真实编辑正文链接，周期从数月压到数天。我们的稀缺性在于"愿意具名说话的中国实体工厂"。
