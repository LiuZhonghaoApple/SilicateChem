...
### ⚠️ 2026-08-01 发现：两个英文站在抢同一批关键词（优先级高于外链）

详见 [`seo/backlinks/2026-08-01-domain-conflict-zzhchem.md`](seo/backlinks/2026-08-01-domain-conflict-zzhchem.md)。

众智名下目前跑着三个站：`zzhchem.cn`（中文官网，工商登记官网 `lgwjg.com` 跳转至此）、**`zzhchem.com`（英文站）**、`silicatechem.com`（英文站，即本仓库）。

**`zzhchem.com` 与本站打的是同一个产品、同一批关键词、同一批买家**：11 个产品页高度重叠（无水/五水偏硅酸钠、液体硅酸钠、硅酸钠），**挂的是同一批证书 PDF（文件名完全一致）**，meta 描述套路相同。两站之间**互无链接**（两边整站 HTML 搜 `silicatechem` 均零匹配）。GSC 账号内**只有 `silicatechem.com` 一个资源**，无人监控二者冲突。

**影响**：关键词自相残杀 + 权重劈成两半——给本站挣的每条外链对 `zzhchem.com` 毫无作用，反之亦然。这可能是本站平均排名卡在第 29 位的原因之一。**在运营做出"哪个站当主力"的决定前，外链投入的效果是打折的。**

**待运营决策**：A 合并（推荐，选一个主站、另一个整站 301）／B 明确分工（按产品线或市场切分，需真动内容）／C 维持现状（不推荐）。**这是生意决定，不由 AI 代做。**

**不依赖上述决策、现在就能做的一件事**：让 `zzhchem.cn` 页头/页脚加一个 "English" 入口指向英文站。中文站链到自家英文出口站是国际站点标准配置，与"migoglass 子站批量互链"性质完全不同（1–2 条 vs 几十条；同公司同产品不同语言）。**但不得扩大为多域名互链网络。**

**另记**：工商信息显示公司带有「被执行人」标签（公开记录）。与本工作的相关性仅在于——个别行业协会/目录收录前会做背景核查，记者做深度稿也可能查到。多数投稿与目录不涉及，但需心中有数。
