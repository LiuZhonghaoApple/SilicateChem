...
const statements = [
  ...
  `CREATE TABLE IF NOT EXISTS media_ai_tasks (
    ...
    asset_type TEXT NOT NULL CHECK (asset_type IN ('image', 'video', 'both')),
    priority TEXT NOT NULL DEFAULT 'normal' CHECK (priority IN ('normal', 'high', 'urgent')),
    brief TEXT NOT NULL,
    evidence_refs JSONB NOT NULL DEFAULT '[]'::JSONB,
    ai_draft JSONB NOT NULL DEFAULT '{}'::JSONB,
    status TEXT NOT NULL DEFAULT 'draft' CHECK (status IN ('draft', 'in_progress', 'human_review', 'pass', 'blocked')),
    reviewer_note TEXT,
    created_by TEXT NOT NULL,
    reviewed_by TEXT,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  ...
  `CREATE TABLE IF NOT EXISTS media_ai_assets (
    id TEXT PRIMARY KEY,
    task_id TEXT NOT NULL REFERENCES media_ai_tasks(id) ON DELETE CASCADE,
    kind TEXT NOT NULL CHECK (kind IN ('image', 'video', 'poster', 'other')),
    ...
    source_status TEXT NOT NULL DEFAULT 'user_provided' CHECK (source_status IN ('user_provided', 'verified', 'illustrative', 'conflict')),
    consent_status TEXT NOT NULL DEFAULT 'pending' CHECK (consent_status IN ('approved', 'pending', 'not_required')),
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
  )`,
  ...
  `CREATE INDEX IF NOT EXISTS media_ai_tasks_status_idx ON media_ai_tasks (status, updated_at DESC)`,
  `CREATE INDEX IF NOT EXISTS media_ai_tasks_created_idx ON media_ai_tasks (created_at DESC)`,
  `CREATE INDEX IF NOT EXISTS media_ai_assets_task_idx ON media_ai_assets (task_id, created_at DESC)`,
];
